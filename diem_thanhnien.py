...
OUTPUT_CSV = "diem_thi_thanhnien.csv"
ERROR_FILE = "errors_thanhnien.txt"
CHECKPOINT_FILE = "checkpoint_thanhnien.txt"
...
def parse_thanhnien_payload(payload: Any):
    """
    Thanh Niên trả JSON dạng:
    {
        "data": [ {"NAM": "2026", "SOBAODANH": "04002032", "DM2": "7.75", ...} ],
        "total": 1,
        "success": true
    }
...
def get_score(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            return parse_thanhnien_payload(payload)
...
def scan_hoi_dong(
    ...
    print(f"Nguồn: Thanh Niên")
    print(f"API params: pageindex={pageindex}, size={size}, type={type_id}")
...
def parse_args():
    ...
    parser.add_argument("--max-workers", type=int, default=40, help="Số request worker mỗi hội đồng. Mặc định: 20")
    parser.add_argument("--hoi-dong-workers", type=int, default=3, help="Số hội đồng chạy song song. Mặc định: 2")
    ...
    parser.add_argument("--pageindex", type=int, default=1, help="Param pageindex của Thanh Niên. Mặc định: 1")
    parser.add_argument("--size", type=int, default=10, help="Param size của Thanh Niên. Mặc định: 10")
    parser.add_argument("--type", dest="type_id", type=int, default=3, help="Param type của Thanh Niên. Mặc định: 3")
    parser.add_argument("--retries", type=int, default=3, help="Retry khi lỗi request/JSON. Mặc định: 2")
